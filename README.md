---
description: >-
  Excited you are here. In this knowledge base we share the most important info
  and tips to help grow, retain, and build a long-term relationship with your
  subscribers.
---

# 👋 Welcome to Prive

{% hint style="info" %}
Use the navigation bar on the left to find answers to your questions. Can't find what you are looking for? Send our support team a note in your Prive in-app support chat or email cs@tryprive.com.
{% endhint %}

## Get Started

We've put together some helpful guides for you to get setup with our product quickly and easily.

{% content-ref url="getting-started/installing-and-onboarding-to-prive.md" %}
[installing-and-onboarding-to-prive.md](getting-started/installing-and-onboarding-to-prive.md)
{% endcontent-ref %}

{% content-ref url="getting-started/migrating-to-prive.md" %}
[migrating-to-prive.md](getting-started/migrating-to-prive.md)
{% endcontent-ref %}

{% content-ref url="creating-and-editing-subscription-plans/creating-a-subscription-plan.md" %}
[creating-a-subscription-plan.md](creating-and-editing-subscription-plans/creating-a-subscription-plan.md)
{% endcontent-ref %}

## Storefront & Customer Portal&#x20;

The basics for a delightful storefront and subscriber portal.&#x20;

{% content-ref url="storefront/setting-up-your-storefront.md" %}
[setting-up-your-storefront.md](storefront/setting-up-your-storefront.md)
{% endcontent-ref %}

{% content-ref url="storefront/editing-your-storefront.md" %}
[editing-your-storefront.md](storefront/editing-your-storefront.md)
{% endcontent-ref %}

{% content-ref url="customer-portal/editing-the-customer-portal.md" %}
[editing-the-customer-portal.md](customer-portal/editing-the-customer-portal.md)
{% endcontent-ref %}

## Prive Admin 101&#x20;

Some quick places to start in the Prive admin&#x20;

{% content-ref url="managing-subscriptions/editing-subscriptions-for-your-subscribers.md" %}
[editing-subscriptions-for-your-subscribers.md](managing-subscriptions/editing-subscriptions-for-your-subscribers.md)
{% endcontent-ref %}

{% content-ref url="broken-reference" %}
[Broken link](broken-reference)
{% endcontent-ref %}
